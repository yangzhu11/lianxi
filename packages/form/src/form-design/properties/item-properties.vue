      ...
      <el-form :key="selectItem.key" v-show="selectItem.key" size="mini" :disabled="disabled" :model="selectItem">
        ...
        <el-form-item label="数据字段" v-if="!hideModel && !noModel.includes(selectItem.type)">
          <el-input
            ...
            placeholder="请输入"
            :disabled="selectItem.item != undefined && selectItem.item.id != undefined"
          />
        </el-form-item>
        ...
        <template v-if="selectItem.type == 'select'">
          ...
          <el-form-item label="联动关联">
            <el-switch v-model="options.linkage" active-text="是" inactive-text="否"> </el-switch>
          </el-form-item>
          ...
          <el-form-item label="实时搜索回调" v-if="options.showSearch && options.onlineSearch">
            <el-input
              type="textarea"
              placeholder="搜索带参,eg: name=$, $表示当前输入关键字"
              v-model="options.onlineParams"
            />
          </el-form-item>
          <el-divider></el-divider>
          ...
          <el-form-item v-if="selectItem.options.clearable" label="清除后回调">
            <el-input
              ...
              v-model="selectItem.options.clearCb"
            />
          </el-form-item>
        </template>
        <!-- select end -->
        ...
        <template v-if="selectItem.type == 'checkbox'">
          ...
          <!-- select 本地配置才有默认值 -->
          <el-form-item v-if="options.dynamic == 0" label="默认值">
            <el-checkbox-group :options="options.options" v-model="options.defaultValue">
              <el-checkbox
                v-for="(checkitem, index) in [].concat(options.options)"
                :label="checkitem.value"
                :key="checkitem.value + index"
              >
                {{ checkitem.label }}
              </el-checkbox>
            </el-checkbox-group>
        ...
        <template v-if="selectItem.type == 'radio'">
          ...
          <!-- select 本地配置才有默认值 -->
          <el-form-item v-if="options.dynamic == 0" label="默认值">
            <el-radio-group :options="options.options" v-model="options.defaultValue">
              <el-radio
                v-for="(checkitem, index) in [].concat(options.options)"
                :label="checkitem.value"
                :key="checkitem.value + index"
        ...
        <template v-if="selectItem.type == 'date' || selectItem.type == 'time' || selectItem.type == 'datePicker'">
          ...
          <el-form-item label="默认值">
            <el-input
              ...
              v-model="options.defaultValue"
              :placeholder="(typeof options.format === 'undefined' ? '' : options.format) + ',当前日期使用now'"
            />
            <el-input
              v-if="(selectItem.type == 'date' || selectItem.type == 'datePicker') && options.range"
              v-model="options.rangeDefaultValue[0]"
              :placeholder="'起始时间' + (typeof options.format === 'undefined' ? '' : options.format)"
            />
            <el-input
              v-if="(selectItem.type == 'date' || selectItem.type == 'datePicker') && options.range"
              v-model="options.rangeDefaultValue[1]"
              :placeholder="'结束时间' + (typeof options.format === 'undefined' ? '' : options.format)"
            />
          ...
          <el-form-item label="时间格式">
            <el-input
              ...
              :placeholder="
                selectItem.type == 'date'
                  ? 'YYYY-MM-DD'
                  : selectItem.type == 'datePicker'
                  ? 'YYYY-MM-DD HH:mm:ss'
                  : 'HH:mm:ss'
              "
            />
          ...
          <el-form-item label="操作属性">
            ...
            <el-checkbox v-model="options.clearable" label="可清除" />
            <el-checkbox
              v-if="selectItem.type == 'date' || selectItem.type == 'datePicker'"
        ...
        <template v-if="selectItem.type == 'batch'">
          ...
          <el-divider class="divider-center">
            {{ options.addType == 'dialog' ? '外部展示字段' : '字段宽度' }}
          </el-divider>
          <el-form-item>
        ...
        <template v-if="selectItem.type == 'text'">
          ...
          <el-form-item label="动态必选">
            <el-input
              type="textarea"
              v-model="options.showRequiredMarkScript"
              :rows="4"
              placeholder="请输入表达式或者动态函数,数据实体以$标识"
            />
          </el-form-item>
        </template>
        <!-- 标签  end -->
        ...
        <template v-if="selectItem.type == 'tree'">
         

          <el-form-item label="树结构需要展开的行数" >
            <el-input
              v-model="selectItem.options.expanded"
              placeholder="请输入"
        ...
        <template v-if="!hideModel && selectItem && selectItem.options">
          <el-form-item label="动态显示">
            ...
            <el-switch v-model="selectItem.options.dynamicVisible" active-text="打开" inactive-text="关闭"> </el-switch>
          </el-form-item>
        ...
        <template v-if="!hideModel && selectItem && selectItem.options && selectItem.options.disabled">
          <el-form-item label="动态禁用">
            <!-- 每个元素都有隐藏条件 根据渲染数据的值来改变 -->
            <el-switch v-model="selectItem.options.dynamicDisabled" active-text="打开" inactive-text="关闭">
            </el-switch>
          </el-form-item>
